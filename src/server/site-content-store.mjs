...
export async function getSiteContentSection(section) {
  const store = readStore();
  const selected = store[section] ?? defaultSiteContent[section];
...
export async function saveSiteContentSection(section, data) {
  const store = readStore();
  store[section] = data;
  writeStore(store);
...
export async function deleteSiteContentSection(section) {
  const store = readStore();
  if (!(section in store)) {
